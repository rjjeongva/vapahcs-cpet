"""
match_icd10_to_cpet.py

Fills the "history of / risk factor" columns in testcpetdb.xlsm using
diagnosis codes from Patients_All_ICD10_Codes.xlsx.
"""
...
ICD10_FILE = "Patients_All_ICD10_Codes.xlsx"
ICD10_SHEET = None          # None = use the first sheet
ICD10_HEADER_ROW = 1

CPET_FILE = "testcpetdb.xlsx"
CPET_SHEET = None           # None = use the first sheet ("Sheet1" in the real file)
CPET_HEADER_ROW = 1

OUTPUT_FILE = "cpet_filled.xlsx"

# Two ICD10 dates within this many days of each other count as one soft
...
# --- Patients_All_ICD10_Codes.xlsx columns ---------------------------------
ICD10_FIRSTNAME_COL = "PatientFirstName"
ICD10_LASTNAME_COL = "PatientLastName"
ICD10_BIRTHDATE_COL = "BirthDateTime"
ICD10_CODE_COL = "ICD10Code"
ICD10_DESC_COL = "ICD10Description"
ICD10_DATE_COL = "Earliest_ICD10Code_Visit_Time"

MAPPING_CSV = "icd10_category_mapping.csv"
...
def load_icd10_patients(path):
    wb = openpyxl.load_workbook(path, data_only=True)
    ws = wb[ICD10_SHEET] if ICD10_SHEET else wb.worksheets[0]
    rows = list(ws.iter_rows(values_only=True))
    header = [str(h).strip() if h is not None else "" for h in rows[ICD10_HEADER_ROW - 1]]
    col = {name: header.index(name) for name in
           [ICD10_FIRSTNAME_COL, ICD10_LASTNAME_COL, ICD10_BIRTHDATE_COL,
            ICD10_CODE_COL, ICD10_DESC_COL, ICD10_DATE_COL] if name in header}

    missing = [n for n in [ICD10_FIRSTNAME_COL, ICD10_LASTNAME_COL, ICD10_BIRTHDATE_COL,
                            ICD10_CODE_COL, ICD10_DESC_COL, ICD10_DATE_COL] if n not in col]
    if missing:
        raise ValueError(f"ICD10 file is missing expected columns: {missing}")

    patients = {}
    for r in rows[ICD10_HEADER_ROW:]:
        if r is None or all(v is None for v in r):
            continue
        first = r[col[ICD10_FIRSTNAME_COL]]
        last = r[col[ICD10_LASTNAME_COL]]
        bdate = to_date(r[col[ICD10_BIRTHDATE_COL]])
        code = r[col[ICD10_CODE_COL]]
        desc = r[col[ICD10_DESC_COL]]
        vdate = to_date(r[col[ICD10_DATE_COL]])
...
def main():
    ...
    icd10_patients = load_icd10_patients(ICD10_FILE)
    name_index = build_name_index(icd10_patients)
    print(f"  {len(icd10_patients)} unique (name, birthdate) patients loaded from ICD10 file.")
    ...
    for block in reversed(blocks):
        ...
        matched_count += 1
        icd_rows = icd10_patients[match_key]
